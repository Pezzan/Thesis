...
\makeatother
  \title{Research on the dosimetric accuracy of pencil beam fine sampling for radiation proton dose calculation}
...
\newenvironment{ackn}%
{\thispagestyle{empty}\null\vspace{\stretch{1}}\begin{center}\textbf{Acknowledgements}\\[20pt]}%
{\end{center}\vspace{\stretch{1}}\null}%

\newenvironment{ackn_it}%
{\thispagestyle{empty}\null\vspace{\stretch{1}}\begin{center}\textbf{Ringraziamenti}\\[20pt]}%
{\end{center}\vspace{\stretch{1}}\null}%
...
\begin{ackn_it}
Un grandissimo ringraziamento va la mia Famiglia, i miei amici e Dio per il supporto e la vicinanza che mi hanno dimostrato, fisicamente e spiritualmente, fancendomi prima di tutto crescere come uomo e poi rendendo possibile tutto questo.

Vorrei ringraziare il Prof. Alberto Del Guerra e il Dr. Mark Bangert, miei relatori, per l'aiuto fornitomi in questi mesi e gli insegnamenti che mi hanno donato, che hanno permesso la mia crescita personale e professionale.

Ringrazio Lucas Burigo, collega del dipartimento di Fisica del DKFZ, per avermi fornito tutto il materiale e l'aiuto di cui avevo bisogno per le simulazioni Monte Carlo. E Ringrazio anche miei colleghi e compagni d'ufficio, Hubert, Niklas, Silke and Hans-Peter per il calore dimostrato e il costante aiuto.

Un rigraziamento particolarmente affettuoso va alla dottoressa Maria Elisabetta Giglio che con la sua dedizione al lavoro, il suo supporto instancabile, una stoica sopportazione del sonno e infinita pazienza, ha permesso a questo lavoro di apparire così ai vostri occhi.
\end{ackn_it}

\begin{ackn}
I want to thank my family, my friends and God for the support and the love they have given to me practically and spiritually, making me grow as a man and letting all this happen.

I thank Prof. Alberto Del Guerra and Dr. Mark Bangert, my supervisors, for helping me during these months and the knowledge they have transmitted to me, giving strong basis for my personal growth as scientist.

I want to thank Lucas Burigo, collegue of mine at the Physics department of the DKFZ, for all the material and the help he has given to me for the Monte Carlo simulations. I also thank all the other collegues, Hubert, Niklas, Silke and Hans-Peter for the constant support.

A special thank goes to Dr. Maria Elisabetta Giglio whose strong dedication, support, patience and love have allowed this work to be as it looks like now.
\end{ackn}

\newpage
\begin{abstract}
Hadrontherapy is a medical therapy based on a new technology allowing to treat cancer without surgery and with contained damages to healthy surrounding tissues. Nowadays, the so-called treatment planning is the core of every therapy and consists in run simulations in order to estimate the dose deposition inside the patient before the real treatment occurs.\\ 
The most accurate algorithms designed for this purpose are Monte Carlo-based and simulate the effects for a huge amount of particles, requiring a significant amount of computing time. New methods have been developed, such as pencil beam algorithms and pencil beam fine sampling. The aim of this project is to understand, improve and implement pencil beam fine sampling by using matRad and to compare different methods with the measurements provided by the Heidelberg Ion-Beam Therapy Center (HIT), Syngo RT planning (the Sienec treatment planning system used for clinical operation at HIT) and Fluka-based MC simulations.\\
The most peculiar property of the pencil beam algorithms is the high speed. For this reason, we have focused on developing a new software based on the pencil beam algorithms which instead allows to increase the accuracy of the simulation. The starting point of this work is the comparison between matRad, fine sampling (FS) algorithm and Monte Carlo simulations run for simple cases considering environments based on both water and inhomogeneities. Then, we show the results of the spread-out Bragg peaks and for full field simulation run with matRad, FS and Syngo.\\
The aim of this thesis is to determine if pencil beam fine sampling is enough accurate and reliable so as to be used in treatment planning. First, we have considered a simple water environment whose aim is to test the method so as to show the presence of any mistakes in the code and the results have proved a very good agreement with Monte Carlo simulations. Second, the software has been tested by considering phantoms with inhomogeneities in order to check how the three methods behave in the cases where the beam encounters a steep lateral gradient (in stopping power) on its path. As last, we have verified the compatibility of our prediction with the real measurements. These are our gold standard and this is the final test that every software has to overcome so as to be considered as a good predictor. Unfortunately, few single beam cases and only one single full field case is not enough to assess the reliability of the software, but it is enough to assert its consistency.\\
What globally emerges from our results is that pencil beam and fine sampling behave in complete opposite ways and that neither of them have yielded substantial results concerning the physical shape of the peak. Pencil beam has been proven to excessively depend on the shape of the phantom, sometimes being a bit contrived. Moreover, fine sampling takes into account too much on the path of the central part of the beam, that means it takes care less of the inhomogeneities nearby the trajectory. We show that pencil beam and pencil beam fine sampling are two methods with significant differences, opposite strengths but almost equivalent in terms of accuracy. The Monte Carlo simulations has been halfway in between pencil beam and fine sampling in almost all the cases. However, the obtained results are consistent with other studies, ensuring us that the our research is going in the right direction.\\
The application possibilities of pencil beam fine sampling are a few, even with all the possible improvements. My proposal is to acquire what we have learned from this work and use it in order to design a new algorithm that does not need ray casting and with an approach more similar to Monte Carlo. From what I have learned during this thesis, I think the world of the dose calculation is moving towards simulations based on Monte Carlo methods and GPU computing. There is the need of novelty in dose calculation and that might be the starting point for the future.
...
\begin{figure}[!b]
\centering
\includegraphics[width=.465\textwidth]{Images/ald_rx2}
\includegraphics[width=.435\textwidth]{Images/ald_rd2}
...
\vspace{-5mm}
{\includegraphics[width=0.6\textwidth]{Images/ray_trac_ex}}
...
\node (geo) [process, below of=rt, yshift=-0.0cm] {Calculate lateral distances};
\node (calcdose) [process, below of=geo, yshift=-0.0cm] {Calculate Dose};

\node (loopbixel) [decision, below of=calcdose, yshift=-1.2cm] {For each PB};
\node (loopray) [decision, below of=loopbixel, yshift=-2.3cm] {For each Ray};

\node (end) [startstop, below of=loopray, yshift=-1cm] {Export dose influence matrix};

\node[coordinate] (r_store) [right of=calcdose, xshift=2cm] {};
\node[coordinate] (r_loopbixel)   [right of=loopbixel, xshift=2cm] {};
\node[coordinate] (bef_dose) [below of=geo, yshift=0.8cm] {};
\node[coordinate] (r_bef_dose) [right of=bef_dose, xshift=2cm] {};
\node[coordinate] (bef_calcdose) [below of=rt, yshift=0.8cm] {};
\node[coordinate] (r_bef_calcdose) [right of=bef_calcdose, xshift=3cm] {};
\node[coordinate] (r_loopray)   [right of=loopray, xshift=3cm] {};

\node[coordinate] (r_phantom) [right of=calcdose, xshift=5cm] {};
\node[coordinate] (l_phantom) [left of=calcdose, xshift=-5cm] {};


\draw [arrow] (start) -- (rt);
\draw [arrow] (rt) -- (geo);
\draw [arrow] (geo) -- (calcdose);
\draw [arrow] (calcdose) -- (loopbixel);
...
\node (loopcomp) [decision, below of=calcdose, yshift=-1.4cm] {For each Comp};
\node (loopbixel) [decision, below of=loopcomp, yshift=-2.6cm] {For each PB};
\node (loopray) [decision, below of=loopbixel, yshift=-2.4cm] {For each Ray};
\node (end) [startstop, right of=loopray, xshift=4cm] {Export Dose Influence Matrix};

\node[coordinate] (l_loopcomp) [left of=loopcomp, xshift=-2cm]{};
\node[coordinate] (bef_weight) [below of=weight, yshift=0.5cm]{};
\node[coordinate] (l_bef_weight) [left of=bef_weight, xshift=-2cm] {};

\node[coordinate] (l_loopbixel)   [left of=loopbixel, xshift=-3cm] {};
\node[coordinate] (bef_geo) [below of=geo, yshift=0.5cm] {};
\node[coordinate] (l_bef_geo) [left of=bef_geo, xshift=-3cm] {};

\node[coordinate] (bef_rt) [below of=rt, yshift=0.5cm]{};
\node[coordinate] (l_bef_rt) [left of=bef_rt, xshift=-4cm]{};
...
\draw [arrow] (calcdose) -- (loopcomp);
\draw [arrow] (loopcomp) -- (loopbixel);
\draw [arrow] (loopbixel) -- (loopray);
\draw [arrow] (loopray) -- (end);

\draw[arrow] (loopcomp) -- (l_loopcomp) -- node[midway, left]{} (l_bef_weight) --(bef_weight) ;
\draw[arrow] (loopbixel) -- (l_loopbixel) -- node[midway, left]{}  (l_bef_geo) -- (bef_geo) ;
\draw[arrow] (loopray) -- (l_loopray) -- node[midway, left]{}  (l_bef_rt) -- (bef_rt) ;
\end{tikzpicture}
...
\vspace{-5mm}
{\includegraphics[width=0.45\textwidth]{Images/prof}}
...
{\includegraphics[width=.32\textwidth]{Images/mR_FSBvsMC_pub_soukup/mR_FSBvsMC_pub_soukup_12}\label{fig:SB1prof_p1}}
\subfloat[][\emph{\scriptsize Peak profile}]
{\includegraphics[width=.32\textwidth]{Images/mR_FSBvsMC_pub_soukup/mR_FSBvsMC_pub_soukup_13}\label{fig:SB1prof_p2}}\\
...
\subfloat[][\emph{\scriptsize Peak profile}]
{\includegraphics[width=.32\textwidth]{Images/mR_FSBvsMC_pub_soukup/mR_FSBvsMC_pub_soukup_25}\label{fig:SB2prof_p1}}
\subfloat[][\emph{\scriptsize Peak profile}]
{\includegraphics[width=.32\textwidth]{Images/mR_FSBvsMC_pub_soukup/mR_FSBvsMC_pub_soukup_26}\label{fig:SB2prof_p2}}\\
...
\centering
\subfloat[][\emph{\scriptsize Central Dose profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_08}} 
\subfloat[][\emph{\scriptsize Central Dose profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_09}}\\
\subfloat[][\emph{\scriptsize Central Dose profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_10}}
\subfloat[][\emph{\scriptsize Integrated Depth Dose profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_11}}\\
\subfloat[][\emph{\scriptsize Entering profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_12}}
\subfloat[][\emph{\scriptsize Peak profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_14}\label{fig:DW1prof_peak}}\\
\subfloat[][\emph{\scriptsize Surface entering profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_15}}
\subfloat[][\emph{\scriptsize Surface peak profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_16}\label{fig:DW1prof_peak2}}\\
...
\subfloat[][\emph{\scriptsize Central Dose profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_25}\label{fig:DW2prof_cdp1}}\\
\subfloat[][\emph{\scriptsize Central Dose profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_26}\label{fig:DW2prof_cdp2}}
\subfloat[][\emph{\scriptsize Integrated Depth Dose profile}]
...
\subfloat[][\emph{\scriptsize Peak profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_30}\label{fig:DW2prof_peak}}\\
\subfloat[][\emph{\scriptsize Surface entering profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_31}}
\subfloat[][\emph{\scriptsize Surface peak profile}]
{\includegraphics[width=.42\textwidth]{Images/mR_FSBvsMC_pub_doublew_single/mR_FSBvsMC_pub_doublew_single_32}\label{fig:DW2prof_peak2}}\\
...
\subsection{Double Wedge}
\label{sec:FFDW}
This section exposes the predictions made with matRad, Syngo and pencil beam fine sampling and shows the comparison between them by using the $\gamma$-index test and difference map. In figure \ref{fig:FFDW} are shown the slices of a sagittal plane of the phantom passing through the isocenter of the CT-cube. The geometry of this phantom is explained in section \ref{sec:dwphant}.
\begin{figure}[!h]
\centering
\subfloat[][\emph{matRad}]
{\includegraphics[width=.35\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_01}}
\subfloat[][\emph{fine sampling}]
{\includegraphics[width=.35\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_02}\label{fig:FFDW_fs}}
\subfloat[][\emph{Syngo}]
{\includegraphics[width=.35\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_03}}
\caption{Full field dose deposition in double wedge phantom. Dose in $Gy$}
\label{fig:FFDW}
\end{figure}
...
\centering
\subfloat[][\emph{Percentage\\ difference}]
{\includegraphics[width=.43\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_06}} 
\subfloat[][\emph{$\gamma$-index \\ $99.91\%$ [$3\%$ $3mm$]}]
{\includegraphics[width=.43\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_07}\label{fig:FFDWgam_ms}} \\
\subfloat[][\emph{Percentage\\ difference}]
{\includegraphics[width=.43\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_04}} 
\subfloat[][\emph{$\gamma$-index \\ $91.65\%$ [$3\%$ $3mm$]}]
{\includegraphics[width=.43\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_05}
\label{fig:FFDWgam_mf}}
\caption{Comparison between fine sampling beam and Syngo Simulations (down), matRad and Syngo Simulations (up) in double wedge phantom, in full field.}
\label{fig:FFDWgam}
\end{figure}
...
\centering
\subfloat[][\emph{\scriptsize Central Dose profile}]
{\includegraphics[width=.48\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_08}\label{fig:FFDWprof_cdp}} 
%\subfloat[][\emph{\scriptsize Central Dose profile}]
%{\includegraphics[width=.42\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_09}\label{fig:FFDWprof_cdp1}}\\
%\subfloat[][\emph{\scriptsize Central Dose profile}]
%{\includegraphics[width=.42\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_10}\label{fig:FFDWprof_cdp2}}
\subfloat[][\emph{\scriptsize Integrated Depth Dose profile}]
{\includegraphics[width=.48\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_11}\label{fig:FFDWprof_ddp}}\\
\subfloat[][\emph{\scriptsize Entering profile}]
{\includegraphics[width=.48\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_12}}
\subfloat[][\emph{\scriptsize Peak profile}]
{\includegraphics[width=.48\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_13}\label{fig:FFDWprof_peak}}\\
\subfloat[][\emph{\scriptsize Surface entering profile}]
{\includegraphics[width=.48\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_14}}
\subfloat[][\emph{\scriptsize Surface peak profile}]
{\includegraphics[width=.48\textwidth]{Images/Images_doublew/mR_FSBvsMC_pub_doublew_15}\label{fig:FFDWprof_peak2}}\\
\caption{Profiles in full field double wedge phantom simulation. The \emph{surface} profile indicates the profile on a plane parallel to the entrering surface.}
\label{fig:FFDWprof}
\end{figure}
...
In the following pages, I will show a series of images showing a slice of the dose cube evaluated with matRad and the projection of the position of the 24 chambers, all in CT resolution. There is also a 3D visualization that shows the position of the chambers inside the area of the deposited dose plotted in form of iso-surfaces corresponding to a dose equal to the $10\%$, $30\%$, $50\%$, $70\%$ and $90\%$ percent of the maximum dose, in scale from blue to red.
Associated to this set of images, there is a graph that reports the actual value in $cGy$ of the ionization chambers for measurements and simulations.
I will comment the results in the next chapter.

\newpage
\begin{figure}[h!]
\centering
\subfloat[][\emph{Sagittal plane \#1}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_01.png}} \quad
\subfloat[][\emph{Sagittal plane \#2}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_04.png}}\\
\subfloat[][\emph{Coronal plane \#1}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_02.png}} 
\subfloat[][\emph{Coronal plane \#2}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_05.png}} \\
\subfloat[][\emph{Ionization chambers values \#1}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_03_new.png}} \quad
\subfloat[][\emph{Ionization chambers values \#2}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_06.png}} \quad
\caption{Dose calculated with matRad, geometry and comparison from configuration 1 and 2}
\label{fig:pos12}
\end{figure}

\begin{figure}[h!]
\centering
\subfloat[][\emph{Sagittal plane \#3}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_07.png}} \quad
\subfloat[][\emph{Sagittal plane \#4}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_10.png}}\\
\subfloat[][\emph{Coronal plane \#3}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_08.png}} 
\subfloat[][\emph{Coronal plane \#4}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_11.png}} \\
\subfloat[][\emph{Ionization chambers values \#3}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_09.png}} \quad
\subfloat[][\emph{Ionization chambers values \#4}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_12.png}} \quad
\caption{Dose calculated with matRad, geometry and comparison from configuration 3 and 4}
\label{fig:pos34}
\end{figure}

\begin{figure}[h!]
\centering
\subfloat[][\emph{Sagittal plane \#5}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_13.png}} \quad
\subfloat[][\emph{Sagittal plane \#6}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_16.png}}\\
\subfloat[][\emph{Coronal plane \#5}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_14.png}} 
\subfloat[][\emph{Coronal plane \#6}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_17.png}} \\
\subfloat[][\emph{Ionization chambers values \#5}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_15.png}} \quad
\subfloat[][\emph{Ionization chambers values \#6}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_18.png}} \quad
\caption{Dose calculated with matRad, geometry and comparison from configuration 5 and 6}
\label{fig:pos56}
\end{figure}

\begin{figure}[h!]
\centering
\subfloat[][\emph{Sagittal plane \#7}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_19.png}} \quad
\subfloat[][\emph{Sagittal plane \#8}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_22.png}}\\
\subfloat[][\emph{Coronal plane \#7}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_20.png}} 
\subfloat[][\emph{Coronal plane \#8}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_23.png}} \\
\subfloat[][\emph{Ionization chambers values \#7}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_21.png}} \quad
\subfloat[][\emph{Ionization chambers values \#8}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_24.png}} \quad
\caption{Dose calculated with matRad, geometry and comparison from configuration 7 and 8}
\label{fig:pos78}
\end{figure}

\begin{figure}[h!]
\centering
\subfloat[][\emph{Sagittal plane \#9}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_25.png}} \\
\subfloat[][\emph{Coronal plane \#9}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_26.png}} \\
\subfloat[][\emph{Ionization chambers values \#9}]
{\includegraphics[width=.47\textwidth]{Images/Meas/Meas_MR_comparison_27.png}} \quad
\caption{Dose calculated with matRad, geometry and comparison from configuration 9}
\label{fig:pos9}
\end{figure}
...
In the case of inhomogeneous phantom, we have seen how the three methods behave in the cases where the beam encounters a steep lateral gradient (in stopping power) on its path. The dose calculated with matRad appears sharply cut into two parts and there is no smooth transition between them. Then, it becomes evident that they look like two completely independent halves of a Bragg peak.
...
\caption{Gamma index in relation to the case number (the case are listed in section \ref{air}), for FS and MR related to table \ref{tab:air}}
\label{fig:gamcase}
\end{figure}
...
As clear in table \ref{tab:FFDW},  in most of the cases, the maximum percentage difference and the standard deviation, between the measured dose and the dose calculated with fine sampling, are globally in line with the others. In particular, fine sampling should be compared with the \emph{planned} dose which should be the most accurate one between the four listed methods. As explained before, Syngo dose was calculated interpolating the full dose deposition on the chamber positions, instead the planned dose was obtained with direct computation.
...
\bottomrule
\end{tabular}
\caption{Results for the full field double wedge. Maximum difference in percentage and standard deviation ($\sigma$) in $cGy$}
\label{tab:FFDW}
\end{table}
...
%In almost all the examples analysed in this thesis, we have seen that fine sampling has obtained results with higher agreement with MC in local dose difference and gamma analysis, with respect to the pencil beam.
