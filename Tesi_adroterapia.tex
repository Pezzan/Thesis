...
\begin{ackn}
andate che vi ci mando!
\end{ackn}

\newpage
\begin{abstract}
Hadrontherapy is a medical therapy based on a new technology allowing to treat cancer without surgery and with contained damages to healthy surrounding tissues. Nowadays, before every treatment, some simulations are run in order to estimate the dose deposition inside the patient. This is called treatment planning and it is one of the core parts of the treatment. 
The most accurate algorithms for this purpose are Monte Carlo based and simulate the effects for a huge amount of particles requiring a significant amount of computing time. New methods have been developed, such as pencil beam algorithms and pencil beam fine sampling. The aim of this project is to understand, improve and implement the last of the aforesaid methods by using matRad, and to compare them with the measurements provided by the Heidelberg Ion-Beam Therapy Center, Syngo RT planning (the Sienec treatment planning system used for clinical operation at HIT) and Fluka-based MC simulations. The most peculiar property of the pencil beam algorithms is the speed, so I worked on a method that allow to increase the accuracy of the simulation instead. Then, my work focuses on the realization of this new software based on the pencil beam algorithms. The first of the three steps of this work is to compare matRad, the fine sampling (FS) algorithm and the Monte Carlo simulations for simple cases, i.e., in water environment and with some inhomogeneities. Then, I show the results for some cases of the spread-out Bragg peaks and finally, for a full field simulation either for matRad, FS and Syngo. 

The aim of these work is to determine if pencil beam fine sampling is enough accurate and reliable. 
So, the first step is to test it in a simple water environment that has the aim to test the method in order to show the presence of any mistakes in the code and has given back a very good agreement with montecarlo. 
After that, the software will be tested in phantoms with inhomogeneities in order to check how the three methods behave in the cases where the beam encounters a steep lateral gradient (in stopping power) on its path.
...
Unfortunately, few single beam cases and only one single full field case is not enough to assess its reliability but enough to check its consinsency.

What we globally see from the results is that pencil beam and fine sampling behave in complete opposite ways. About the physical shape of the peak, we can assert that none of the two methods have yielded substantial results. For the reasons that I wil explained in this thesis, pencil beam has been proven to depend too much on the shape of the phantom, sometimes being a bit contrived. Moreover, fine sampling takes into account too much on the path of the central part of the beam, that means it cares less about the inhomogeneities nearby the trajectory.
We will see that pencil beam and pencil beam fine sampling are two methods with significant differences, with opposite strengths but almost equivalent in accuracy. And that the Monte Carlo simulations has been halfway in between pencil beam and fine sampling, in almost all the cases.

Furthermore, all our deductions were compared with important articles in this field. The obtained results are in line with other studies, so it ensures us that the research we have made is in the right direction.

In conclusion, I think that, even the good results obtained, the application possibilities of pencil beam fine sampling are a few, even with all the possible improvements. My proposal is to acquire what we have learned from this work and use it in order to design a new algorithm that does not need ray casting and with an approach more similar to Monte Carlo. From what I have learned during this thesis, I think the world of the dose calculation is moving towards simulations based on Monte Carlo methods and GPU computing. There is the need of novelty in dose calculation and that might be the starting point.
...
\subsection{Double Wedge}
\label{sec:FFDW}
This section exposes the predictions made with matRad, Syngo and pencil beam fine sampling and shows the comparison between them by using the $\gamma$-index test and difference map. In figure \ref{fig:FFDW} are shown the slices of a sagittal plane of the phantom passing through the isocenter of the CT-cube.
\begin{figure}[!h]
\centering
\subfloat[][\emph{matRad}]
...
\caption{Profiles in full field double wedge phantom simulation.}
\label{fig:FFDWprof}
\end{figure}
...
\caption{Results, geometry and comparison from configuration 3 and 4}
\label{fig:pos34}
\end{figure}
...
\caption{Results, geometry and comparison from configuration 5 and 6}
\label{fig:pos56}
\end{figure}
...
\caption{Results, geometry and comparison from configuration 7 and 8}
\label{fig:pos78}
\end{figure}
